...
public class MainActivity extends Activity {
 ...
 save.setOnClickListener(v->{if(user.getText().toString().trim().isEmpty()){user.setError("Enter username");user.requestFocus();return;}if(pass.length()==0){pass.setError("Enter password");pass.requestFocus();return;}AutoLogin.prefs(this).edit().putString("username",user.getText().toString().trim()).putString("password",pass.getText().toString()).putBoolean("enabled",mode.isChecked()).remove("lastNetwork").remove("pin").apply();NetworkEvents.sync(this);requestLoginNotifications();((android.view.inputmethod.InputMethodManager)getSystemService(INPUT_METHOD_SERVICE)).hideSoftInputFromWindow(pass.getWindowToken(),0);if(!mode.isChecked())login();else Toast.makeText(this,"Saved",Toast.LENGTH_SHORT).show();});
