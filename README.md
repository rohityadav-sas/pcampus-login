...
The app supports installation from Android 6-16. This **Root** variant needs Magisk or another working `su` provider. Its root helper listens for Wi-Fi address changes through Android's `ip monitor` command and wakes the app only when an address changes; it needs no permanent notification.
